        ...
        with admin_tabs[1]:
            ...
            if st.button("Find Best Resumes", key="find_best_resumes"):
                ...
                if filtered_resumes:
                    ...
                    st.subheader(f"Top {num_resumes} Resumes")
                    ...
                    # Select button with a unique key
                    if st.button("Select These Resumes", key="select_filtered_resumes"):
                ...
                if job_position:
                    ...
                    if matched_field:
                        ...
                        if field_resumes:
                            ...
                            st.subheader(f"Top {job_num_resumes} Candidates for {job_position}")
                            top_df = pd.DataFrame([
                                {
                                    'ID': r['id'], 
                                    'Name': r['name'], 
                            ...
                            # Select button with a unique key
                            if st.button("Select These Candidates", key="select_job_candidates"):
                                selected_ids = [r['id'] for r in st.session_state.job_search_results]
                                if update_selected_status(cursor, connection, selected_ids):
                                    st.success(f"Successfully selected {len(selected_ids)} top candidates.")
                                    st.rerun()
                        else:
